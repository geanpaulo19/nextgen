...
window.addEventListener('load', function () {
  ...
  /* ── LOADER ─────────────────────────────────────────────── */
  var lw1 = document.getElementById('lw1'), lw2 = document.getElementById('lw2');
  var lbar = document.getElementById('lbar'), lnum = document.getElementById('lnum');
  var loader = document.getElementById('loader');
  ...
  var loaderInterval = setInterval(function () {
    ...
    if (pct >= 100) {
      clearInterval(loaderInterval);
      gsap.to([lw1, lw2], { y: '0%', duration: 0.85, stagger: 0.1, ease: 'power4.out', delay: 0.1 });
      gsap.to(loader, {
        opacity: 0, duration: 0.65, delay: 1.2,
        onComplete: function () { loader.style.display = 'none'; initHero(); }
      });
    }
  }, 65);
  ...
  /* ── HERO ────────────────────────────────────────────────── */
  function initHero() {
    gsap.to(['#hw1', '#hw2'], { y: '0%', duration: 1.05, stagger: 0.1, ease: 'power4.out' });
    gsap.to('#eyebrow',    { opacity: 1, y: 0, duration: 0.75, delay: 0.45, ease: 'power3.out' });
    gsap.to('#heroBot',    { opacity: 1, y: 0, duration: 0.75, delay: 0.65, ease: 'power3.out' });
    gsap.to('#scrollHint', { opacity: 1, duration: 0.6, delay: 1.0 });
    gsap.to('.hero-bg',       { yPercent: 25, ease: 'none', scrollTrigger: { trigger: '#hero', start: 'top top', end: 'bottom top', scrub: 1 } });
    gsap.to('.hero-gridlines',{ yPercent: 12, ease: 'none', scrollTrigger: { trigger: '#hero', start: 'top top', end: 'bottom top', scrub: 1.5 } });
  }

  /* ── FADE-UP ─────────────────────────────────────────────── */
  document.querySelectorAll('.fade-up').forEach(function (el) {
    gsap.to(el, { opacity: 1, y: 0, duration: 0.8, ease: 'power3.out', scrollTrigger: { trigger: el, start: 'top 89%', once: true } });
  });

  /* ── WORK ROWS ───────────────────────────────────────────── */
  gsap.fromTo('.wrow', { opacity: 0, x: -20 }, { opacity: 1, x: 0, stagger: 0.08, duration: 0.65, ease: 'power3.out', scrollTrigger: { trigger: '.works-list', start: 'top 82%', once: true } });

  /* ── HORIZONTAL PIN ──────────────────────────────────────── */
  var track = document.getElementById('htrack'), pfill = document.getElementById('hpfill');
  var slides = document.querySelectorAll('.hslide');
  function getMaxScroll() { return (slides[0].offsetWidth + 24) * (slides.length - 1); }
  ScrollTrigger.create({
    trigger: '#hpin', start: 'top top',
    end: function () { return '+=' + (getMaxScroll() + window.innerWidth * 0.4); },
    pin: '#hsticky', scrub: 1,
    onUpdate: function (self) {
      gsap.set(track, { x: -getMaxScroll() * self.progress });
